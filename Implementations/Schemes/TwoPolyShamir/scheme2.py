# scheme2.py. This is "C:\Users\harma\OneDrive - City, University of London\Cyber Security MSc\Individual Project\Implementations of schemes\Shamirs\2PolyLowerPrimeForHigherThreshold.py"
...
def generate_location_shares(latitude, longitude, minimum, shares, prime=_PRIME, scale=10000):
    """Generates Shamir shares for both latitude and longitude."""
...
def recover_location(lat_shares, lon_shares, prime=_PRIME, scale=10000):
    """Recover both latitude and longitude from their respective shares."""
    latitude = recover_secret(lat_shares, prime, scale)
    longitude = recover_secret(lon_shares, prime, scale)
    return latitude, longitude

def make_random_shares(secret, minimum, shares, prime=_PRIME, scale=10000):
    """Generates a random Shamir pool for a given secret (with decimals), returns share points."""
    if minimum > shares:
...
def recover_secret(shares, prime=_PRIME, scale=10000):
    """Recover the secret from share points and scale it back to the original decimal value."""
    if len(shares) < 2:
...
def _extended_gcd(a, b):
    """Extended Euclidean algorithm for finding the inverse."""
    x, last_x = 0, 1
    y, last_y = 1, 0
    while b != 0:
        quot = a // b
